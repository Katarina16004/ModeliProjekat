...
{	
	...
	public class ModelResourcesDesc
	{
		...
		{
            // 1. BaseVoltage i Container-i (oni ne zavise ni od koga)
            typeIdsInInsertOrder.Add(ModelCode.BASEVOLTAGE);
            typeIdsInInsertOrder.Add(ModelCode.CONNECTIVITYNODECONTAINER);
            typeIdsInInsertOrder.Add(ModelCode.TOPOLOGICALNODE);

            // 2. ConnectivityNode (zavisi od kontejnera i toponoda)
            typeIdsInInsertOrder.Add(ModelCode.CONNECTIVITYNODE);

            // 3. Switch (zavisi od BaseVoltage)
            typeIdsInInsertOrder.Add(ModelCode.SWITCH);

            // 4. Terminal (zavisi i od Switcha i od ConnectivityNode-a)
		...
		private void InitializeNotSettablePropertyIds()
		{
            ...
            notSettablePropertyIds.Add(ModelCode.CONNNODECONTAINER_NODES);
            notSettablePropertyIds.Add(ModelCode.TOPOLOGICALNODE_CONNNODES);

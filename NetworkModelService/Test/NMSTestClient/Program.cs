...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            tgda.GetSwitchWithMaxGid();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetSwitchWithMaxGid failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "6")
                    {
                        try
                        {
                            tgda.GetTerminalsForSwitchWithMaxGid();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetTerminalsForSwitchWithMaxGid failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Get Switch with max GID");           // DODATO
            Console.WriteLine("\t6) Get Terminals for Switch with max GID");  // DODATO
